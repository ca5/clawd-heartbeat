...
A physical Claude Code status indicator: M5Atom Lite firmware (PlatformIO/C++) + a hook script. The LED shows session state through a 3D-printed figure's heart window. Four transports carry the same one-line commands: HTTP over WiFi, USB serial, BLE via a custom GATT service (for networks where the host can't reach the Atom), or HOGP — BLE HID — for managed machines whose MDM policy blocks custom GATT UUIDs. BLE and HOGP each need a resident daemon (`ble-bridge.py` / `hid-bridge.py`, uv-run); the HOGP one is thinner because the OS HID driver owns the link.

## Helping the user

- First-time device setup → use the `setup` skill (interactive: transport choice, WiFi, flash, hooks)
- All four transports are on `main`. Pick HOGP when the host is a managed machine whose MDM blocks custom GATT UUIDs (check `HKLM\SOFTWARE\Microsoft\PolicyManager\current\device\Bluetooth`), plain BLE otherwise
...
- `src/main.cpp` — firmware: per-session state, priority aggregation (`wait > err > done > tool > idle`), all colors/timings. `applyLed` / `applyRgb` / `statusBody` / `handleLine` are shared by the HTTP handlers, the USB serial parser (`pollSerial`), and the BLE characteristic writes (`pollBle`, drained from an SPSC ring that both the NUS `RxCallbacks` and the HOGP `HidOutputCallbacks` fill, so state aggregation is transport-agnostic). WiFi is optional (empty SSID) and non-blocking; `BLE_ENABLED` toggles the BLE peripheral and `HID_ENABLED` the HID service alongside it
- `led.sh` — hook script (copy lives at `~/.claude/led.sh`): transport switch (`ATOM_BLE` > `ATOM_SERIAL` > `ATOM_URL`), session_id extraction, AskUserQuestion→wait, dialog-wait marker (subagent overwrite protection), send timestamp, transcript watcher that catches denials/interrupts (which fire no hook event). Transport precedence is `ATOM_HID` > `ATOM_BLE` > `ATOM_SERIAL` > `ATOM_URL`; HOGP falls back to serial (never to HTTP, whose placeholder IP would cost a 1s curl timeout per event). BLE/HOGP modes write lines to the respective daemon's socket via the shared `sock_write` and autostart it; `led.sh ensure-ble` (wired into the SessionStart hook) starts whichever is configured up front. `ATOM_BLE_PORT` / `ATOM_HID_PORT` (auto-set on Git Bash/MSYS) switch the socket to loopback TCP, written via bash's `/dev/tcp`
- `hid-bridge.py` — HOGP daemon (hidapi, uv/PEP723). Same socket protocol as `ble-bridge.py`, so `led.sh` treats them alike, but it holds no BLE link of its own: the OS HID driver does, so there is no keepalive, reconnect or sleep-resume handling. Writes command lines as 64-byte Output Reports and reassembles `status` from Input Reports. `--scan` lists the vendor-defined HID collections. Listens on `127.0.0.1:47821` on Windows
...
- `led-test.sh` — manual testing (`status` / `states` / `rgb` / `ramp` / `coupon`); device address from `ATOM` env or gitignored `.atom-ip` — `hid` (or `hid:<sock>` / `hid:<port>`) means HOGP, `ble` likewise means BLE, `auto` finds the serial port, `/dev/...` is an explicit serial port, anything else is an HTTP URL
...
- `BLEHIDDevice::manufacturer(std::string)` writes to a characteristic the constructor never creates — calling it panics the board into a boot loop (`LoadProhibited`). The no-arg `manufacturer()` is the one that creates it, so use `manufacturer()->setValue(...)`. Only `pnp()`, `hidInfo()` and `reportMap()` are safe to call directly
- The HID report characteristics are created with `ESP_GATT_PERM_*_ENCRYPTED`, so a host touching HID must pair. NUS characteristics keep their open defaults, which is what lets the Mac stay on plaintext NUS while a managed Windows box uses HOGP — measured working simultaneously (`ble=connected n=2`). Don't add global permission changes that would drag NUS into requiring encryption
- Advertising with HOGP is 29 of 31 bytes (flags 3 + HID 16-bit UUID 4 + Appearance 4 + NUS 128-bit UUID 18). `BLEAdvertisementData::addData` silently drops anything that would overflow, so the HID entries are added *first*: if something later pushes it over, the NUS UUID is what disappears and `ble-bridge.py` can still find the device by name
...
- Daemon liveness (`sock_alive` in `led.sh`, `ble_alive` in `atom-antigravity.sh`) must actually connect, never just `[ -S "$sock" ]`. A daemon that dies without cleanup leaves a socket file that refuses connections; treating it as alive makes `ensure_*_daemon` return early, so the daemon is never restarted and the LED goes silent with no error and exit code 0. Don't have the shell delete the stale socket either — both bridges `os.unlink` it before bind, and a shell-side `rm` races with a daemon that just started. The probe is a python `AF_UNIX` connect, not `nc`: macOS's `nc` returns non-zero for `-U -z` even against a live socket (measured), which would make every check report dead and spawn duplicate daemons
