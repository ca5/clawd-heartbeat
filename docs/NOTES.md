...
**「OS がリンクを保持してくれる」は成り立たなかった(2026-09-10)**。HOGP を選ぶ理由の一つに
「再接続やスリープ復帰を OS 任せにできる」を挙げていたが、**入力トラフィックの無いベンダー定義
HID は Windows がアイドルで切る**。ボンドは残るのにリンクだけ落ち、HID コレクションも消える
(デバイスは広告を出し続けているのに `connection_status=0`)。対策として hid-bridge.py が
`maintain_connection` の `GattSession` を掴んだままにする。掴んでいる間だけリンクが上がり、
離すと即座に切れることを実測で確認した。キーボードやマウスが切れないのは入力を送り続けて
いるからで、こちらは黙っているので同じ扱いにはならない。
...
IO が無いので UI 操作なしに完了する)。さらに **hid-bridge.py が自動で復旧する**: セッションを
